...
def playground(request):
    return render(request, "weblearning/pages/playground.html",  {'title': "Playground"})

@login_required
def count(request):
    return render(request, "weblearning/pages/count.html",  {'title': "Count"})

@login_required
def table(request):
    return render(request, "weblearning/pages/table.html",  {'title': "Table"})
...
def docs(request, doc_type: str):
    ...
    valid_docs = [ os.path.splitext(f)[0] for f in  os.listdir(docs_path) if os.path.isfile(os.path.join(docs_path, f))]
